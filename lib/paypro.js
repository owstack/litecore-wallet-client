...
PayPro.get = function(opts, cb) {
  ...
  opts.headers = opts.headers || {
    'Accept': ltcPayPro.PAYMENT_REQUEST_CONTENT_TYPE,
  ...
  http(opts, function(err, dataBuffer) {
    ...
    try {
      var body = ltcPayPro.PaymentRequest.decode(dataBuffer);
      request = (new ltcPayPro()).makePaymentRequest(body);
    ...
    var decodedDetails = ltcPayPro.PaymentDetails.decode(serializedDetails);
    var pd = new ltcPayPro();
    ...
    var addr = new ltcLib.Address.fromScript(new ltcLib.Script(scriptBuf), network);
...
PayPro._getPayProRefundOutputs = function(addrStr, amount) {
  ...
  var output = new ltcPayPro.Output();
  var addr = new ltcLib.Address(addrStr);

  var s;
  if (addr.isPayToPublicKeyHash()) {
    s = ltcLib.Script.buildPublicKeyHashOut(addr);
  } else if (addr.isPayToScriptHash()) {
    s = ltcLib.Script.buildScriptHashOut(addr);
  } else {
...
PayPro.send = function(opts, cb) {
  ...
  opts.method = 'POST';
  opts.headers = opts.headers || {
    'Accept': ltcPayPro.PAYMENT_ACK_CONTENT_TYPE,
    'Content-Type': ltcPayPro.PAYMENT_CONTENT_TYPE,
    // 'Content-Type': 'application/octet-stream',
  };
  opts.body = payment;
  ...
  http(opts, function(err, rawData) {
    ...
    if (rawData) {
      try {
        var data = ltcPayPro.PaymentACK.decode(rawData);
        var pp = new ltcPayPro();
