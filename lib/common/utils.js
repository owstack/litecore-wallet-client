...
var crypto = ltcLib.crypto;
var encoding = ltcLib.encoding;
...
Utils.hashMessage = function(text) {
  ...
  ret = new ltcLib.encoding.BufferReader(ret).readReverse();
  return ret;
};
...
Utils.privateKeyToAESKey = function(privKey) {
  ...
  $.checkArgument(ltcLib.PrivateKey.isValid(privKey), 'The private key received is invalid');
  var pk = ltcLib.PrivateKey.fromString(privKey);
  return ltcLib.crypto.Hash.sha256(pk.toBuffer()).slice(0, 16).toString('base64');
};
...
Utils.deriveAddress = function(scriptType, publicKeyRing, path, m, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new ltcLib.HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      ltcAddress = Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      ltcAddress = Address.fromPublicKey(publicKeys[0], network);
      break;
  }
...
Utils.verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new ltcLib.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
Utils.formatAmount = function(satoshis, unit, opts) {
  ...
  var amount = clipDecimals((satoshis / u.toLitoshis), u[precision].maxDecimals).toFixed(u[precision].maxDecimals);
...
Utils.buildTx = function(txp) {
  ...
  if (txp.toAddress && txp.amount && !txp.outputs) {
    ...
    _.each(txp.outputs, function(o) {
      ...
      if (o.script) {
        t.addOutput(new ltcLib.Transaction.Output({
          script: o.script,
          satoshis: o.amount
        }));
      } else {
