...
var BASE_URL = 'http://localhost:3232/ltcws/api';
...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  function testHardcodedKeys() {
    ...
    var xpub = ltcLib.HDPublicKey.fromString('xpub6BosfCnifzxcFwrSzQiqu2DBVTshkCXacvNsWGYJVVhhawA7d4R5WSWGFNbi8Aw6ZRc1brxMyWMzG3DSSSSoekkudhUd9yLb6qx39T9nMdj');
  ...
  function testLiveKeys() {
    ...
    if (!xpriv) {
      xpriv = new ltcLib.HDPrivateKey(c.xPrivKey);
    }
    xpriv = xpriv.deriveChild(c.getBaseAddressDerivationPath());
    var xpub = new ltcLib.HDPublicKey(c.xPubKey);
...
API.prototype._import = function(cb) {
  ...
  // First option, grab wallet info from LTCWS.
...
API.prototype.decryptBIP38PrivateKey = function(encryptedPrivateKeyBase58, passphrase, opts, cb) {
  ...
  var privateKey = new ltcLib.PrivateKey(privateKeyWif);
  var address = privateKey.publicKey.toAddress().toString();
  var addrBuff = new Buffer(address, 'ascii');
  var actualChecksum = ltcLib.crypto.Hash.sha256sha256(addrBuff).toString('hex').substring(0, 8);
  var expectedChecksum = ltcLib.encoding.Base58Check.decode(encryptedPrivateKeyBase58).toString('hex').substring(6, 14);
...
API.prototype.getBalanceFromPrivateKey = function(privateKey, cb) {
  ...
  var privateKey = new ltcLib.PrivateKey(privateKey);
...
API.prototype.buildTxFromPrivateKey = function(privateKey, destinationAddress, opts, cb) {
  ...
  async.waterfall([
    ...
    function(utxos, next) {
      ...
      var tx;
      try {
        var toAddress = ltcLib.Address.fromString(destinationAddress);

        tx = new ltcLib.Transaction()
          .from(utxos)
          .to(toAddress, amount)
          .fee(fee)
          .sign(privateKey);
...
API.prototype._getHeaders = function(method, url, args) {
  var headers = {
    'x-client-version': 'ltcwc-' + Package.version,
  };
...
API._buildSecret = function(walletId, walletPrivKey, network) {
  if (_.isString(walletPrivKey)) {
    walletPrivKey = ltcLib.PrivateKey.fromString(walletPrivKey);
  }
  var widHex = new Buffer(walletId.replace(/-/g, ''), 'hex');
  var widBase58 = new ltcLib.encoding.Base58(widHex).toString();
...
API.parseSecret = function(secret) {
  ...
  try {
    ...
    var widHex = ltcLib.encoding.Base58.decode(widBase58).toString('hex');
    var walletId = split(widHex, [8, 12, 16, 20]).join('-');

    var walletPrivKey = ltcLib.PrivateKey.fromString(secretSplit[1]);
...
API.signTxp = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new ltcLib.HDPrivateKey(derivedXPrivKey);
...
API.prototype._addSignaturesToLtcTx = function(txp, t, signatures, xpub) {
  if (signatures.length != txp.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new ltcLib.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = txp.inputs[i];
    try {
      var signature = ltcLib.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(txp.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: ltcLib.crypto.Signature.SIGHASH_ALL,
        publicKey: pub,
      };
...
API.prototype._applyAllSignatures = function(txp, t) {
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToLtcTx(txp, t, x.signatures, x.xpub);
  });
};
...
API.prototype.createWallet = function(walletName, copayerName, m, n, opts, cb) {
  ...
  var walletPrivKey = opts.walletPrivKey || new ltcLib.PrivateKey();
  ...
  var args = {
    ...
    m: m,
    n: n,
    pubKey: (new ltcLib.PrivateKey(walletPrivKey)).toPublicKey().toString(),
    network: network,
...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var c = self.credentials;
    var walletPrivKey = ltcLib.PrivateKey.fromString(c.walletPrivKey);
...
API.signTxProposalFromAirGapped = function(key, txp, unencryptedPkr, m, n, opts) {
  ...
  var newClient = new API({
    baseUrl: 'https://ltcws.example.com/ltcws/api'
  });
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var reqPrivKey = new ltcLib.PrivateKey(opts.generateNewKey ? null : this.credentials.requestPrivKey);
  var requestPubKey = reqPrivKey.toPublicKey().toString();

  var xPriv = new ltcLib.HDPrivateKey(this.credentials.xPrivKey)
