...
Credentials.create = function(network) {
  ...
  x.network = network;
  x.xPrivKey = (new ltcLib.HDPrivateKey(network)).toString();
...
Credentials.fromExtendedPublicKey = function(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  var x = new Credentials();
  x.xPubKey = xPubKey;
  x.entropySource = ltcLib.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
Credentials.prototype._hashFromEntropy = function(prefix, length) {
  ...
  var b2 = ltcLib.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
Credentials.prototype._expand = function() {
  ...
  if (this.xPrivKey) {
    var xPrivKey = new ltcLib.HDPrivateKey.fromString(this.xPrivKey);
  ...
  if (this.entropySourcePath) {
    ...
    this.entropySource = ltcLib.crypto.Hash.sha256sha256(seed).toString('hex');
  }

  if (this.entropySource) {
    // request keys from entropy (hw wallets)
    var seed = this._hashFromEntropy('reqPrivKey', 32);
    var privKey = new ltcLib.PrivateKey(seed.toString('hex'), network);
    ...
    this.entropySource = ltcLib.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
...
Credentials.prototype.getDerivedXPrivKey = function(password) {
  ...
  var xPrivKey = new ltcLib.HDPrivateKey(this.getKeys(password).xPrivKey, this.network);
  var deriveFn = !!this.compliantDerivation ? _.bind(xPrivKey.deriveChild, xPrivKey) : _.bind(xPrivKey.deriveNonCompliantChild, xPrivKey);
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    // IN LTCWS, the master Pub Keys are not sent to the server, 
    ...
    var privKey = new ltcLib.PrivateKey.fromBuffer(ltcLib.crypto.Hash.sha256(seedBuf));
  ...
  var pkr = _.map(w.publicKeyRing.copayersExtPubKeys, function(xPubStr) {
    ...
    if (isMe) {
      ...
      requestDerivation = (new ltcLib.HDPrivateKey(credentials.xPrivKey))
        .deriveChild(path).hdPublicKey;
    } else {
      // this 
      var path = Constants.PATHS.REQUEST_KEY_AUTH;
      requestDerivation = (new ltcLib.HDPublicKey(xPubStr)).deriveChild(path);
    }

    // Grab Copayer Name
    var hd = new ltcLib.HDPublicKey(xPubStr).deriveChild('m/2147483646/0/0');
